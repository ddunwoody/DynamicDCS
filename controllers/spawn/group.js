...
_.set(exports, 'spawnSupportBaseGrp', function ( serverName, baseName, side ) {
	...
	var farpBases = _.filter(curBases, (baseObj)=> {
		return ((_.includes(baseObj._id, '_MOB') && _.get(baseObj, 'initSide') === side)|| _.includes(_.get(baseObj, '_id'), '_FOB')) && _.first(_.split(_.get(baseObj, 'name'), ' #')) === baseName;
	});
...
_.set(exports, 'spawnLogisticCmdCenter', function (serverName, staticObj, init, baseObj, side) {
	...
	_.set(curGrpObj, 'type', 'Shelter');
	_.set(curGrpObj, 'shape_name', 'ukrytie');
